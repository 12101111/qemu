...
static void set_cpu_aarch64(ARMCPU *cpu, bool aarch64, uint64_t entry_point) {
  CPUARMState *env = &cpu->env;
  if (aarch64) {
    env->aarch64 = true;
    env->pstate = PSTATE_MODE_EL1h; // start at EL1
    for (int i = 0; i < 31; i++) {
      env->xregs[i] = 0; // clear register
    }
    env->cp15.sctlr_el[1] = 0;       // disable MMU
    env->cp15.hcr_el2 = 0;           // disable EL2
    env->cp15.scr_el3 = 0;           // disable EL3
  } else if (entry_point % 2 == 0) { // A32 mode
    env->aarch64 = false;
    env->thumb = false;
    for (int i = 0; i < 16; i++) {
      env->regs[i] = 0; // clear register
    }
  } else { // T32 mode
    env->aarch64 = false;
    env->thumb = true;
...
static void wa2x_machine_state_init(MachineState *machine) {
  ...
  /* ROM reset vector */
  set_cpu_aarch64(&s->cpu, aarch64, elf_entry);
  // bootinfo.ram_size = machine->ram_size;
  // bootinfo.loader_start = wa2x_memmap[WA2X_ROM].base;
