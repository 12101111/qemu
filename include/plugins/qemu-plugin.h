 ...
 *
 * version 7 (fork):
 * - added qemu_plugin_register_vcpu_mem_cond_cb
 * - added qemu_plugin_register_vcpu_insn_exec_cond_inline_per_vcpu
 * - added qemu_plugin_register_vcpu_mem_cond_inline_per_vcpu
 */
...
/**
 * qemu_plugin_register_vcpu_insn_exec_cond_inline_per_vcpu() - conditional
 * insn exec inline op
 * @insn: the opaque qemu_plugin_insn handle for an instruction
 * @op: the type of qemu_plugin_op (e.g. ADD_U64)
 * @entry: entry to run op
 * @imm: the op data (e.g. 1)
 * @cond: condition to enable inline op
 * @cond_entry: first operand for condition
 * @cond_imm: second operand for condition
 *
 * Insert an inline op that is executed each time the instruction executes,
 * but only if @cond_entry @cond @cond_imm is true.
 * If condition is QEMU_PLUGIN_COND_ALWAYS, condition is never interpreted and
 * this function is equivalent to
 * qemu_plugin_register_vcpu_insn_exec_inline_per_vcpu.
 * If condition QEMU_PLUGIN_COND_NEVER, condition is never interpreted and
 * inline op is never installed.
 */
QEMU_PLUGIN_API
...
/**
 * qemu_plugin_register_vcpu_mem_cond_cb() - register conditional memory cb
 * @insn: handle for instruction to instrument
 * @cb: callback of type qemu_plugin_vcpu_mem_cb_t
 * @flags: does the plugin read or write the CPU's registers?
 ...
 * @userdata: any plugin data to pass to the @cb?
 *
 * The @cb function is called on a memory access generated by the
 * instruction if entry @cond imm is true.
 * If condition is QEMU_PLUGIN_COND_ALWAYS, condition is never interpreted and
 * this function is equivalent to qemu_plugin_register_vcpu_mem_cb.
 * If condition QEMU_PLUGIN_COND_NEVER, condition is never interpreted and
 * callback is never installed.
 */
QEMU_PLUGIN_API
...
/**
 * qemu_plugin_register_vcpu_mem_cond_inline_per_vcpu() - conditional inline
 * op for mem access
 * @insn: handle for instruction to instrument
 * @rw: apply to reads, writes or both
 * @op: the op, of type qemu_plugin_op
 ...
 * @cond_imm: second operand for condition
 *
 * This registers an inline op on every memory access generated by the
 * instruction, but only if @cond_entry @cond @cond_imm is true.
 * If condition is QEMU_PLUGIN_COND_ALWAYS, condition is never interpreted and
 * this function is equivalent to
